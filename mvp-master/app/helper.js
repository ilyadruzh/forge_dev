...
for (let key in ORGS) {
	if (key.indexOf('org') === 0 || key == config.myOrganisation) {
		let client = new hfc();

		let cryptoSuite = hfc.newCryptoSuite();
		let path1 = getKeyStoreForOrg(ORGS[key].name);
		cryptoSuite.setCryptoKeyStore(hfc.newCryptoKeyStore({path: path1}));
...
function setupPeers(channel, org, client) {
	for (let key in ORGS[org].peers) {
		let path1 = path.join(__dirname, config.CC_SRC_PATH, ORGS[org].peers[key]['tls_cacerts']);
...
function newOrderer(client) {
	...
	let data = fs.readFileSync(path.join(__dirname, config.CC_SRC_PATH, caRootsPath));
...
function getKeyStoreForOrg(org) {
    ...
    return path.join(__dirname,config.CC_SRC_PATH,hfc.getConfigSetting('keyValueStore'),org);
...
function newRemotes(names, forPeers, userOrg) {
	...
	for (let idx in names) {
		...
		if (ORGS[userOrg].peers[peerName]) {
			...
			let path1 = path.join(__dirname, config.CC_SRC_PATH, ORGS[userOrg].peers[peerName]['tls_cacerts'])
			let data = fs.readFileSync(path1);
			let grpcOpts = {
				pem: Buffer.from(data).toString(),
...
var getRegisteredUsers = function(username, userOrg, isJson) {
	var member;
	var client = getClientForOrg(userOrg);
	var keyStore = getKeyStoreForOrg(getOrgName(userOrg));
...
var getOrgAdmin = function(userOrg) {
	...
	var keyPath = path.join(__dirname, config.CC_SRC_PATH, admin.key);
	var keyPEM = Buffer.from(readAllFiles(keyPath)[0]).toString();
	var certPath = path.join(__dirname, config.CC_SRC_PATH, admin.cert);
...
var setupChaincodeDeploy = function() {
	process.env.GOPATH = path.join(__dirname, "..", "chaincode", "go");
        //__dirname;// path.join(__dirname, hfc.getConfigSetting('CC_SRC_PATH'));
};
